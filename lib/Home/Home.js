...
export default function Home() {
    const { page, setPage, cart, setRouteStack, routeStack, categories, selectedCategory } = useContext(AppContext);
    ...
    const categoriesWithProduct = categories.map((category, _) => {
        return (
            {
                heading: category,
                products: dummyProducts.products.filter(product => product.categories === category).slice(0, 4)
            }
        )
    })
    return (
