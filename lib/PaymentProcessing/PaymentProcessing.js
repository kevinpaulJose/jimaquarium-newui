...
import { Dimensions, Linking, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
import { AppContext, ordersAPI } from "../../App";
import { colorPalete, paymentMethod } from "../constants";
import makeApiRequest from "../api";
import { currency, navItems, removeData, retrieveData, routePrevious, routeTo } from "../utils";
import { Icon } from "../Shared/Icon";
import LottieView from "lottie-react-native";

export default function PaymentProcessing() {
  const { setSnackSeverity, setOpenSnackbarContent, setOpenSnackbar, setSelectedOrder,
    setRouteStack, routeStack, setPage, orders, setOrders
  ...
  const [currentPaymentState, setCurrentPaymentState] = useState("Payment Processing")
  ...
  const setAndRedirect = ({ order }) => {
    ...
    routeTo({ setRouteStack, routeStack, setPage, newPage: navItems[7], stepper: true });
  }
    ...
    if (orderIdCurrent) {
      try {
        ...
        if (res?.status === 200) {
          ...
          await ordersAPI(setSnackSeverity, setOpenSnackbar, setOpenSnackbarContent, setOrders);

          setAndRedirect({ order: { ...orders.filter(o => o.orderId === orderIdCurrent.orderId)[0], paymentStatus: "Failed" } })
    ...
    if (paymentLink) {
      if (paymentMethod.sabPaise) {
        if (orderIdCurrent) {
          ...
          console.log(res?.orders?.filter(o => {
          ...
          try {
            if (isActive) {
              if (paymentStatus === "Paid" || paymentStatus === "Failed") {
                ...
                await removeData({ key: "startTime" });
                await ordersAPI(setSnackSeverity, setOpenSnackbar, setOpenSnackbarContent, setOrders);
                setAndRedirect({ order: orders.filter(o => o.orderId === orderIdCurrent.orderId)[0] })
              } else {
                setTimeout(checkPayment, 10000)
              }
        ...
        if (isActive) {
          if (paymentStatus === "PAID") {
            ...
            if (resUpdate?.status == 200) {
              await ordersAPI(setSnackSeverity, setOpenSnackbar, setOpenSnackbarContent, setOrders);
              console.log("Setting order - ", orders.filter(o => o.orderId === orderIdCurrent.orderId)[0]);
              setAndRedirect({ order: orders.filter(o => o.orderId === orderIdCurrent.orderId)[0] })
            } else {
              await setPaymentFailed();
              setTimeout(checkPayment, 10000)
    ...
    <ScrollView style={styles.container} automaticallyAdjustKeyboardInsets>
      <View style={styles.parentContainer}>
        ...
        <View style={styles.processingContainer}>
          <View>
            <LottieView style={styles.lottieStyle} loop={true} autoPlay source={require('../../assets/loading.json')} />
          </View>
          ...
          <Text style={styles.timerText}>Time Elapsed: {formatTime(remainingTime)}</Text>
        </View>
        <View style={styles.footerContiner}>
          <Pressable onPress={redirectPayment} style={styles.retryContainer}>
            ...
            <Icon name={"sync"} size={23} action={() => { }} active={false} badge={0} color={colorPalete.silver_silk} />
