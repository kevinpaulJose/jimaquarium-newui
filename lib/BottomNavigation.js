...
export default function BottomNavigation() {
  ...
  const validateForm = () => {
    let validatedName = name.length !== 0;
    let validatedLine1 = line1.length !== 0;
    let validatedLine2 = line2.length !== 0;
    let validatedCity = city.length !== 0;
    let validatedState = userState.length !== 0;
    let validatedPin = pin.length !== 0;
    let validatedPhone = phone.length !== 0 && phone.length === 10;
    ...
    if (
      ...
      setOpenSnackbarContent("Please enter valid phone number");
      setOpenSnackbar(true);
      return false;
    }
    setSnackSeverity("warning");
    setOpenSnackbarContent("Please fill all the fields");
    setOpenSnackbar(true);
    return false;
    // console.log(validatedName, validatedLine1, validatedLine2, validatedCity, validatedState, validatedPhone, validatedPin);
  };
  const updateAddress = async () => {
    if (!addressSaving) {
      ...
      if (isFormValid) {
        ...
        let addedAddressIndex = addressList.findIndex(
          (v) => v.id === selectedAddress?.id
        );
    ...
    <>
      ...
      {page.page === "cart" && cart.length > 0 ? (
        <View style={styles.bottomContainer}>
          ...
          <View style={styles.checkoutContainer}>
            <View>
              <Text style={styles.labelTextTotal}>Total</Text>
          ...
          >
            <Text style={styles.checkoutButtonText}>
              {productCartSaving ? "Adding..." : "Proceed to Checkout"}{" "}
            </Text>
          </Pressable>
        </View>
      ) : (
          ...
          {parseInt(selectedProduct.stock) === 0 ? (
            ...
            <Pressable onPress={addProductToCart} style={styles.button}>
              <Text style={styles.buttonText}>
                {productCartSaving ? "Adding..." : "Add to Cart"}
              </Text>
            </Pressable>
          )}
        </View>
          ...
          {selectedPaymentMethod === "visa" ? (
            <Pressable onPress={placeOrder} style={styles.checkoutButton}>
              <Text style={styles.checkoutButtonText}>
                {loadingEnabled
                  ? `Placing order...`
                  : `Proceed to Pay ${currency.INR}${getCurrentTotal()}`}
              </Text>
            </Pressable>
          ) : (
            <Pressable onPress={placeOrder} style={styles.checkoutButton}>
              <Text style={styles.checkoutButtonText}>
                {loadingEnabled ? `Placing order...` : `Buy now`}
              </Text>
            </Pressable>
          )}
        </View>
      ...
      {page.page === "orderDetail" &&
        ...
        selectedOrder?.tracking && (
          <View style={{ ...styles.container, backgroundColor: "white" }}>
            <Pressable
              onPress={() => Linking.openURL(selectedOrder?.tracking)}
              style={styles.checkoutButton}
            >
              <Text style={styles.checkoutButtonText}>{`Track here`}</Text>
            </Pressable>
          </View>
        )}
      ...
      {page.page === "profile" && !userDetails.name && (
        <View style={styles.bottomContainer}>
          <Pressable
            onPress={() => login({ loggingIn, setLoggingIn })}
            style={styles.checkoutButton}
          >
            <Text style={styles.checkoutButtonText}>
              {loggingIn ? "Please wait..." : "Login / Sign up"}
            </Text>
          </Pressable>
        </View>
      )}
      ...
      {products.length === 0 && (
        <View style={styles.bottomContainer}>
          ...
          >
            <Text numberOfLines={2} style={styles.checkoutButtonText}>
            ...
            {products.length === 0 && !productLoading && (
              ...
              >
                Please restart the app if it takes too long
              </Text>
            )}
          </Pressable>
        </View>
        ...
        >
          <View>
            <View style={styles.consentLabel}>
              <Text style={styles.consentText}>{consent.label}</Text>
            </View>
