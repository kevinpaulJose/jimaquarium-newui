...
export default function Orders() {
  ...
  const getStatusColor = ({ status, date }) => {
    switch (status) {
      ...
      case "Cancelled":
        return { color: colorPalete.danger, text: "Cancelled" };
      default:
        return {
          color: colorPalete.lima,
          text: `Estimated Shipping on ${new Intl.DateTimeFormat("en-US", {
            day: "numeric",
            month: "long",
          }).format(new Date(date).setDate(new Date(date).getDate() + 2))}`,
        };
    }
  };
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      {orders.map((order, index) => (
        <React.Fragment key={`${order.id}-${index}`}>
          <Pressable
            onPress={() => setAndRedirect({ order: order })}
            style={styles.container}
          >
            <View style={styles.textContainer}>
              <Text style={styles.orderTitleText}>{`${order.orderId}`}</Text>
              <Text style={styles.orderDescText}>
                {`${new Intl.DateTimeFormat("en-US", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                }).format(new Date(order.updated))}`}
              </Text>
            ...
            <View style={styles.imageContainer}>
              <Image
                ...
                source={{
                  uri: products.filter(
                    (p) => p.productId === order.products[0].id
                  )[0]?.img,
                }}
              />
